...
- `get_file_content` MCP tool — read full file contents by path without needing a symbol name
- Re-export detection in `list_document_symbols` — barrel/index files now return `re_exports` array and `note` field
- Re-export extraction in TS/JS parsers — `export * from` and `export { } from` patterns are indexed as Module symbols

### Fixed

- Fixed clippy redundant_closure warning in main.rs
